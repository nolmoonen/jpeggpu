...
jpeggpu::status read_sos(const uint8_t **image, const uint8_t *image_end,
  ...
  for (int i = 0; i < decoder->num_components; ++i) {
    ...
    decoder->image[i] = new uint8_t[decoder->sizes_x[i] * decoder->sizes_y[i]];
  }
    ...
    for (int yyy = 0; yyy < decoder->ss_y_max; ++yyy) {
      ...
      for (int xx = 0; xx < size_xx; ++xx) {
        for (int xxx = 0; xxx < decoder->ss_x_max; ++xxx) {
          ...
          const uint8_t cy = decoder->image[0][y * decoder->size_x + x];
          const uint8_t cb = decoder->image[1][yy * size_xx + xx];
          const uint8_t cr = decoder->image[2][yy * size_xx + xx];

          const float r = cy + 1.402f * (cr - 128.f);
          const float g =
              cy - .344136f * (cb - 128.f) - .714136f * (cr - 128.f);
